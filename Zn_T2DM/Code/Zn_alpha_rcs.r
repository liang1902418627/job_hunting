...
# 保存路径
fig_dir <- "Results/RCS_Zn_alpha/figures"
...
fml_fpg_m1 <- as.formula(
    paste0(
        "Richness ~ rcs(Serum_Zn, ", k_fixed1, ") + ",
...
p_overall_fpg_m1 <- an_fpg_m1["Serum_Zn", "P"]
...
pred_fpg_m1 <- as.data.frame(
    Predict(fit_fpg_m1, Serum_Zn, conf.int = 0.95, ref.zero = TRUE)
)

# 竖线：暴露中位数（与 ref.zero 的参考点保持一致：datadist 默认也通常取中位数）
inset_v1 <- median(md_v2$Serum_Zn, na.rm = TRUE)

# 保存路径（沿用前面 fig_dir）
if (!dir.exists(fig_dir)) dir.create(fig_dir, recursive = TRUE)

pdf_file1 <- file.path(fig_dir, "Fig1_RCS_SerumZn_Richness_model1_knots3_overlay.pdf")

# 提取 Predict 输出列（兼容不同列名）
x1 <- as.numeric(pred_fpg_m1$Serum_Zn %||% pred_fpg_m1$x)
...
p_overall_fpg_m2 <- an_fpg_m2["Serum_Zn", "P"]
p_nonlinear_fpg_m2 <- an_fpg_m2[" Nonlinear", "P"]

# Predict：连续结局——输出“相对参考值(默认=datadist的Adjust to，通常为中位数)”的 β 及95%CI
pred_fpg_m2 <- as.data.frame(
    Predict(fit_fpg_m2, Serum_Zn, conf.int = 0.95, ref.zero = TRUE)
)

# 竖线：暴露中位数（与 ref.zero 的参考点保持一致：datadist 默认也通常取中位数）
inset_v2 <- median(md_v2$Serum_Zn, na.rm = TRUE)

# 保存路径（沿用前面 fig_dir）
if (!dir.exists(fig_dir)) dir.create(fig_dir, recursive = TRUE)

pdf_file2 <- file.path(fig_dir, "Fig2_RCS_SerumZn_Shannon_model3_knots3_overlay.pdf")

# 提取 Predict 输出列（兼容不同列名）
x2 <- as.numeric(pred_fpg_m2$Serum_Zn %||% pred_fpg_m2$x)
...
# 竖线：暴露中位数（与 ref.zero 的参考点保持一致：datadist 默认也通常取中位数）
inset_v3 <- median(md_v2$Serum_Zn, na.rm = TRUE)

# 保存路径（沿用前面 fig_dir）
if (!dir.exists(fig_dir)) dir.create(fig_dir, recursive = TRUE)
pdf_file3 <- file.path(fig_dir, "Fig3_RCS_SerumZn_Pielou_model3_knots3_overlay.pdf")
...
p_overall_fpg_m4 <- an_fpg_m4["Serum_Zn", "P"]
p_nonlinear_fpg_m4 <- an_fpg_m4[" Nonlinear", "P"]
# Predict：连续结局——输出“相对参考值(默认=datadist的Adjust to，通常为中位数)”的 β 及95%CI
pred_fpg_m4 <- as.data.frame(
    Predict(fit_fpg_m4, Serum_Zn, conf.int = 0.95, ref.zero = TRUE)
)
# 竖线：暴露中位数（与 ref.zero 的参考点保持一致：datadist 默认也通常取中位数）
inset_v4 <- median(md_v2$Serum_Zn, na.rm = TRUE)
# 保存路径（沿用前面 fig_dir）
if (!dir.exists(fig_dir)) dir.create(fig_dir, recursive = TRUE)
pdf_file4 <- file.path(fig_dir, "Fig4_RCS_SerumZn_Simpson_model3_knots3_overlay.pdf")
# 提取 Predict 输出列（兼容不同列名）
x4 <- as.numeric(pred_fpg_m4$Serum_Zn %||% pred_fpg_m4$x)
