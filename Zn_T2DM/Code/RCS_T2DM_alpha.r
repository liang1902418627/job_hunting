# -------------------------------------------
# 题目：糖尿病与多样性指数的RCS图片
# 自变量：alha多样性指数（Richness, Shannon, Pielou, Simpson）
# 因变量：糖尿病OR
# 协变量：模型3 中的协变量
...
nrow(md_v2)
write.csv(md_v2, file = "Data/Zn_T2DM_alpha.csv", row.names = FALSE)
# 缺失值检查
sum(is.na(md_v2$Pielou))

# 设置RCS回归的参数, 先不做IFG的，IFG单独跑
outcome_vars <- c("T2DM") # 血清锌, 连续变量
exposure_vars <- c("Richness", "Shannon", "Pielou", "Simpson") # 因变量全是连续变量
...
# ---------------------------
# 单独验证：Richness -> T2DM，k=3，协变量=模型3
# ---------------------------
...
# 拟合（T2DM 为二分类时用 lrm）
fit_single <- rms::lrm(fml, data = md_v2)

# 查看模型与AIC
print(fit_single)
cat("AIC(single) =", AIC(fit_single), "\n")

# anova 与 P 值提取（复用你上面写的 get_rcs_pvals）
...
# Richness vs T2DM--------------------------------------------
cov1 <- model_covariates$model3

k_fixed1 <- 5
fml_fpg_m1 <- as.formula(
    paste0(
        "T2DM ~ rcs(Richness, ", k_fixed1, ") + ",
        paste(cov1, collapse = " + ")
    )
)
print(fml_fpg_m1)
fit_fpg_m1 <- lrm(fml_fpg_m1, data = md_v2, x = TRUE, y = TRUE)

an_fpg_m1 <- anova(fit_fpg_m1)
p_overall_fpg_m1 <- an_fpg_m1["Richness", "P"]
p_nonlinear_fpg_m1 <- an_fpg_m1[" Nonlinear", "P"]

# Predict：二分类结局——输出“相对参考值(默认=datadist的Adjust to，通常为中位数)”的  OR 及95%CI
pred_fpg_m1 <- as.data.frame(
    Predict(fit_fpg_m1, Richness, conf.int = 0.95, fun = exp, ref.zero = TRUE)
)

# 竖线：暴露中位数（与 ref.zero 的参考点保持一致：datadist 默认也通常取中位数）
inset_v1 <- median(md_v2$Richness, na.rm = TRUE)
# 保存路径（沿用前面 fig_dir）
fig_dir <- "Results/RCS_T2DM_alpha"
...
plot_rcs_hist_overlay(
    ...
    xlab = "Richness",
    ylab_left = expression("T2DM" ~ "OR" ~ " (95% CI)"),
    inset_v = inset_v1,
    col_line = "#93C8ED",
    col_ribbon = adjustcolor("#93C8ED", alpha.f = 0.35),
...
# Shannon vs T2DM--------------------------------------------
cov2 <- model_covariates$model3

k_fixed2 <- 3
fml_fpg_m2 <- as.formula(
    paste0(
        "T2DM ~ rcs(Shannon, ", k_fixed2, ") + ",
        paste(cov2, collapse = " + ")
    )
)
print(fml_fpg_m2)
fit_fpg_m2 <- lrm(fml_fpg_m2, data = md_v2, x = TRUE, y = TRUE)
an_fpg_m2 <- anova(fit_fpg_m2)
p_overall_fpg_m2 <- an_fpg_m2["Shannon", "P"]
p_nonlinear_fpg_m2 <- an_fpg_m2[" Nonlinear", "P"]
# Predict：二分类结局——输出“相对参考值(默认=datadist的Adjust to，通常为中位数)”的  OR 及95%CI
pred_fpg_m2 <- as.data.frame(
    Predict(fit_fpg_m2, Shannon, conf.int = 0.95, fun = exp, ref.zero = TRUE)
)

# 竖线：暴露中位数（与 ref.zero 的参考点保持一致：datadist 默认也通常取中位数）
inset_v2 <- median(md_v2$Shannon, na.rm = TRUE)
pdf_file2 <- file.path(fig_dir, "Fig2_RCS_Shannon_model3_knots3_overlay.pdf")
# 提取 Predict 输出列（兼容不同列名）
...
fml_fpg_m3 <- as.formula(
    paste0(
        "T2DM ~ rcs(Pielou, ", k_fixed3, ") + ",
        paste(cov3, collapse = " + ")
    )
)
print(fml_fpg_m3)
fit_fpg_m3 <- lrm(fml_fpg_m3, data = md_v2, x = TRUE, y = TRUE)
an_fpg_m3 <- anova(fit_fpg_m3)
p_overall_fpg_m3 <- an_fpg_m3["Pielou", "P"]
p_nonlinear_fpg_m3 <- an_fpg_m3[" Nonlinear", "P"]
# Predict：二分类结局——输出“相对参考值(默认=datadist的Adjust to，通常为中位数)”的  OR 及95%CI
pred_fpg_m3 <- as.data.frame(
    Predict(fit_fpg_m3, Pielou, conf.int = 0.95, fun = exp, ref.zero = TRUE)
)

# 竖线：暴露中位数（与 ref.zero 的参考点保持一致：datadist 默认也通常取中位数）
inset_v3 <- median(md_v2$Pielou, na.rm = TRUE)

pdf_file3 <- file.path(fig_dir, "Fig3_RCS_Pielou_model3_knots3_overlay.pdf")
# 提取 Predict 输出列（兼容不同列名）
...
# Simpson vs T2DM--------------------------------------------
cov4 <- model_covariates$model3
k_fixed4 <- 3
fml_fpg_m4 <- as.formula(
    paste0(
        "T2DM ~ rcs(Simpson, ", k_fixed4, ") + ",
        paste(cov4, collapse = " + ")
    )
)

print(fml_fpg_m4)
fit_fpg_m4 <- lrm(fml_fpg_m4, data = md_v2, x = TRUE, y = TRUE)
an_fpg_m4 <- anova(fit_fpg_m4)
p_overall_fpg_m4 <- an_fpg_m4["Simpson", "P"]
p_nonlinear_fpg_m4 <- an_fpg_m4[" Nonlinear", "P"]
# Predict：二分类结局——输出“相对参考值(默认=datadist的Adjust to，通常为中位数)”的  OR 及95%CI
pred_fpg_m4 <- as.data.frame(
    Predict(fit_fpg_m4, Simpson, conf.int = 0.95, fun = exp, ref.zero = TRUE)
)
# 竖线：暴露中位数（与 ref.zero 的参考点保持一致：datadist 默认也通常取中位数）
inset_v4 <- median(md_v2$Simpson, na.rm = TRUE)
pdf_file4 <- file.path(fig_dir, "Fig4_RCS_Simpson_model3_knots3_overlay.pdf")
# 提取 Predict 输出列（兼容不同列名）
