...
# 2) 读取数据并合并 -------------------------------------------------------
# imputed_df_v6: 应含 SampleID、暴露、两个结局
...
# 3) 设定变量（不校正协变量） ---------------------------------------------
# 中介变量（菌属）
g_vars <- c("g_Akkermansia", "g_Prevotella", "g_Leptothrix", "g_Streptococcus")
...
# 4) 通用单次中介分析函数（不校正协变量） -----------------------------------
run_one_mediation <- function(data, mediator, y_var, x_var,
                              control_val, treat_val, sims = 1000) {
  # 仅保留当次分析需要的变量，并去除其中缺失
  vars_needed <- c("SampleID", mediator, y_var, x_var)
  df <- data %>%
  ...
  # 结局模型：仅中介 + 暴露（不校正协变量）
  rhs_y <- paste(c(mediator, x_var), collapse = " + ")
...
# 5) 针对“单个结局”的批量运行封装（各自独立去缺失，不校正协变量） --------------
run_for_outcome <- function(data, y_var, g_vars, x_var, contrasts, sims = 1000) {
  ...
  for (cn in names(contrasts)) {
    ...
    for (m in g_vars) {
      cat(sprintf("运行中介（不校正协变量）：Outcome=%s | %s vs %s | Mediator=%s\n",
                  y_var, treat_val, control_val, m))
      res <- tryCatch(
        run_one_mediation(
          data = data_y, mediator = m, y_var = y_var, x_var = x_var,
...
# diabetes：仅去除 diabetes 缺失（不校正协变量）
res_diabetes <- run_for_outcome(
  data = dt_analysis, y_var = y_diabetes,
  g_vars = g_vars, x_var = x_var,
  contrasts = contrasts, sims = 1000
)

# IFG_or_diabetes：仅去除 IFG_or_diabetes 缺失（不校正协变量）
...
# 7) 导出结果 -------------------------------------------------------------
# 同时创建可能用到的目录，避免写文件时报错
dir.create("结果/分开结局", recursive = TRUE, showWarnings = FALSE)
dir.create("主要结果/表格/中介分析（补充）", recursive = TRUE, showWarnings = FALSE)

readr::write_csv(res_diabetes, file = "主要结果/表格/中介分析（补充）/不校正协变量/mediation_diabetes_Mn_quartiles.csv")
readr::write_csv(res_ifg,      file = "主要结果/表格/中介分析（补充）/不校正协变量/mediation_IFG_or_diabetes_Mn_quartiles.csv")
